...
function parseMarkdown(md, relativePath) {
    ...
    html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, function(match, alt, src) {
        var resolvedSrc = resolveUrl(src, relativePath);
        return '<img src="' + resolvedSrc + '" alt="' + alt + '" class="article-hero" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 20px auto; box-shadow: 0 4px 15px rgba(0,0,0,0.3);">';
    });
...
if (typeof process !== 'undefined' && process.argv && process.argv.length > 2 && typeof require !== 'undefined') {
    var fs = require('fs');
    ...
    if (fs.existsSync(inputPath)) {
        ...
        var htmlContent = parseMarkdown(mdContent, relPath);
