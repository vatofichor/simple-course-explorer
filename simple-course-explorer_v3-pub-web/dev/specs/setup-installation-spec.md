...
3. **Drafting & Publishing Conventions:**
   - Raw articles are stored in `/content_source/` inside section-prefixed folders (e.g., `01_Foo/`).
   - New pages default to draft state (`<!-- ready: false -->`) to prevent incomplete publishing. Toggling **Ready / Published** in the editor writes `<!-- ready: true -->` to compile the file and add it to the Table of Contents.
4. **Media Asset Management:**
   - Authors upload media files (images, PDFs, media) directly via the editor panel's Section Assets box.
   - The uploader saves files inside the page's source section directory (e.g., `/content_source/01_Foo/`).
   - The compiler automatically syncs these assets to `/public/content/01_Foo/`. They are referenced in markdown relative to the section (e.g., `![chart](chart.png)`).
5. **Contribution Metadata:**
   - Authors can toggle "AI-Generated Content" or list "Reviewed Contributors" in the editor checkboxes/inputs. This writes comment metadata tags (`<!-- ai-generated: true/false -->` and `<!-- contributors: ... -->`) that render as highlight badges in the student viewer.
