...
if (file_exists($manifestFile)) {
    $lessons = json_decode(file_get_contents($manifestFile), true) ?: [];
}

// Gather statistics and add type/contributors properties
$aiGenerated = 0;
$humanModified = 0;

foreach ($lessons as &$lesson) {
    $htmlFile = $publicContentDir . '/' . $lesson['relative_path'];
    $lesson['type'] = 'generated'; // default fallback
    $lesson['contributors'] = '';
    
    if (file_exists($htmlFile)) {
        $htmlContent = file_get_contents($htmlFile);
        if (preg_match('/<article[^>]*data-modified=["\']([^"\']+)["\']/i', $htmlContent, $matches)) {
            $lesson['type'] = 'modified';
            $lesson['contributors'] = $matches[1];
            $humanModified++;
        } else {
            $aiGenerated++;
        }
...
$sections = [];
foreach ($lessons as $lesson) {
    $sections[$lesson['section']][] = $lesson;
}
            ...
            <div class="main-panel">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card" style="border-color: rgba(152, 195, 121, 0.4);">
                        <div class="stat-val" style="color: var(--accent-green);"><?php echo $humanModified; ?></div>
                        <div class="stat-label">Reviewed Lessons</div>
                    </div>
                    <div class="stat-card" style="border-color: rgba(224, 108, 117, 0.4);">
                        <div class="stat-val" style="color: var(--text-bright);"><?php echo $aiGenerated; ?></div>
